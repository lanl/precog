...
fit_model_wrapper = function(input_vector){
  ...
  sim_ts                             <- foreach(i=1:N,
   ...
   xx                                 <- length(types_of_curves[rep(c(1:length(types_of_curves)), each = floor(num_curves/1000))])
  ...
  for (test_idx in 1:length(sim_ts_test)){
    ...
    if(full_testing_count > 50) break
	...
	sim_nb1 <- matrix(rnbinom(5000*length(point),mu = (point+1), size = 1*optimal_k_1$par*dispersion_forecast),ncol=length(point),byrow = T)
        sim_nb2 <- matrix(rnbinom(5000*length(point),mu = (point+1), size = 0.5*optimal_k_2$par*dispersion_forecast),ncol=length(point),byrow = T)
        sim_nb3 <- matrix(rnbinom(5000*length(point),mu = (point+1), size = 0.25*optimal_k_3$par*dispersion_forecast),ncol=length(point),byrow = T)
        sim_nb4 <- matrix(rnbinom(5000*length(point),mu = (point+1), size = 0.125*optimal_k_4$par*dispersion_forecast),ncol=length(point),byrow = T)
