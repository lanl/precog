...
ggplot_data = mae_comparison_data %>% dplyr::group_by(model.y) %>% dplyr::summarize(abs_error_model= mean(abs_error_model), 
                                                                                    abs_error_smoa = mean(abs_error_smoa))
ggplot_data = subset(ggplot_data, subset=((abs_error_model<16500)&(abs_error_smoa<10000)) )

triangle_data = data.frame(abs_error_model = c(-Inf, -Inf, 0, 10000),
                           abs_error_smoa = c(Inf, -Inf, -Inf, Inf),
                           model.y = rep(NA, times = 4))
point_plot_1 <- ggplot(ggplot_data,
       aes(x=abs_error_model,y=abs_error_smoa, color=model.y)) + geom_point(size = 4)+
  ...
  geom_point(
    data = subset(ggplot_data, (model.y%in%models_to_label)), # Highlighted points
  ...
  geom_label_repel(data = ggplot_data, aes(label = ifelse(model.y%in%models_to_label,as.character(model.y),'')),
                   box.padding   = 0.35, 
                   point.padding = 0.5,
  ...
  annotate("text", x = min(ggplot_data$abs_error_model), y = max(ggplot_data$abs_error_smoa), 
...
ggplot_data = wis_comparison_data %>% dplyr::group_by(model.y) %>% dplyr::summarize(wis_error_model= mean(wis_error_model), 
                                                                                    wis_error_smoa = mean(wis_error_smoa))
ggplot_data = subset(ggplot_data, subset=((wis_error_model<16500)&(wis_error_smoa<10000)) )
triangle_data = data.frame(wis_error_model = c(-Inf, -Inf, 0, 10000),
                           wis_error_smoa = c(Inf, -Inf, -Inf, Inf),
                           model.y = rep(NA, times = 4))

point_plot_2 = ggplot(ggplot_data,
       aes(x=wis_error_model,y=wis_error_smoa, color=model.y)) + geom_point(size = 4)+
  theme(panel.grid = element_blank(),
        panel.border = element_blank()) +  # Add abline
  ...
  annotate("text", x = min(ggplot_data$wis_error_model), y = max(ggplot_data$wis_error_smoa)+1000, 
           label = "Model Outperforms sMOA", hjust = 0, vjust = 1, size=6) +  
  annotate("text", x = max(ggplot_data$wis_error_model)-1000, y = min(ggplot_data$wis_error_smoa)+1000, 
           label = "Model Outperformed by sMOA", hjust = 1, vjust = 0, size=6) +
  scale_x_continuous(limits=c(0,16500),expand = c(0,0),
                     oob = scales::oob_keep) +
  scale_y_continuous(limits=c(0, 10000),expand = c(0,0),
...
p1 <- ggplot(rank_graph_data, aes(x=abs_error_rank, color=Model)) + 
  ...
  theme(panel.spacing = unit(0, "cm"),
        plot.margin = margin(0, 0, 0, 0, "cm"), 
        plot.caption = element_blank()) + theme(legend.text=element_text(size=15)) + ylab('Density') + 
  xlab('Relative MAE Rank at Date')
...
p2 <- ggplot(rank_graph_data, aes(x=wis_error_rank, color=Model)) + 
  ...
  theme(panel.spacing = unit(0, "cm"),
        plot.margin = margin(0, 0, 0, 0, "cm"), 
        plot.caption = element_blank()) + theme(legend.text=element_text(size=15)) + ylab('Density') + 
  xlab('Relative WIS Rank at Date')


p1 <- p1 + annotate("segment", 
                    x = 0.49, y = 1.8,             # Start of the arrow
                    xend = 0.05, yend = 1.8,       # End of the arrow
                    arrow = arrow(length = unit(0.3, "cm")),  # Arrowhead size
                    color = "darkgrey") +
                    annotate("text", 
                      x = 0.5, y = 1.8,           # Position of the text
...
blank_space <- grid::rectGrob(gp = grid::gpar(col = NA))
lst_p = list(p1, blank_space, p2)
grid.arrange(lst_p[[1]], lst_p[[2]], lst_p[[3]], heights = c(1.1, 0.15, 1.1))
...
p1 <- ggplot(graph_data, aes(x = time, y = value)) + geom_line(linewidth = 1)+ 
  ...
  theme(panel.spacing = unit(0, "cm"),
        plot.margin = margin(0, 0, 0, 0, "cm"), 
        plot.caption = element_blank()) + 
  ylab("Counts/Proportions") + xlab("Time")
