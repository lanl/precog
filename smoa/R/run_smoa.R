...
embed_mat                       <- create_embed_matrix(sim_ts,h=(h),k=(k+1))
X                               <- embed_mat[[1]]
y                               <- embed_mat[[2]]
...
parfctn = function(x){
  ...
  library(collapse)
  library(dplyr)
  longer_quantiles <- c(0.01, 0.025, seq(0.05, 0.95, by = 0.05), 0.975, 0.99)
  ...
  for (location in c(curr_state)){
	...
	est_intervals <- quantile(sim_nb1[,1],probs = longer_quantiles)
        ...
        temp_row <- data.frame(State = location, Date = curr_targetend_date, 
                               ...
                               Coverage = coverage,
			       Level = longer_quantiles[quantile_idx+1])
      ...
      # Gather coverage data for this horizon:
      for(quantile_idx in 0:10){
        est_intervals <- quantile(sim_nb2[,2],probs = longer_quantiles)
        est_intervals <- point[curr_horizon] + (est_intervals-est_intervals[longer_quantiles==0.5])
	lower_quan <- est_intervals[1+quantile_idx]
        upper_quan <- est_intervals[length(longer_quantiles)-quantile_idx]
        coverage <- as.numeric((data_future[curr_horizon]>lower_quan)&
      ...
      # Gather coverage data for this horizon:
      for(quantile_idx in 0:10){
        est_intervals <- quantile(sim_nb3[,3],probs = longer_quantiles)
        est_intervals <- point[curr_horizon] + (est_intervals-est_intervals[longer_quantiles==0.5])
        lower_quan <- est_intervals[1+quantile_idx]
        upper_quan <- est_intervals[length(longer_quantiles)-quantile_idx]
        coverage <- as.numeric((data_future[curr_horizon]>lower_quan)&
      ...
      # Gather coverage data for this horizon:
      for(quantile_idx in 0:10){
        est_intervals <- quantile(sim_nb4[,4],probs = longer_quantiles)
        est_intervals <- point[curr_horizon] + (est_intervals-est_intervals[longer_quantiles==0.5])
        lower_quan <- est_intervals[1+quantile_idx]
        upper_quan <- est_intervals[length(longer_quantiles)-quantile_idx]
        coverage <- as.numeric((data_future[curr_horizon]>lower_quan)&
