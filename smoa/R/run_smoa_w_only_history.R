...
dispersion_forecast_scaling <- seq(from=10, to=100, length.out = 8)
dispersion_forecast <- dispersion_forecast_scaling[sim_idx%%8+1]
mle_lower_bound_scaling <- seq(from=0.05, to=10, length.out = 8)
mle_lower_bound <- dispersion_forecast_scaling[floor((sim_idx-1)/8)+1]
...
name_of_change <- paste("wOnlyHistories_k", k, "num_curves", num_curves, "closest", closest, "dispersion", round(dispersion_forecast*10000), 'mlebound', round(mle_lower_bound*10000), sep = "_")
...
parfctn = function(x){
  ...
  for (location in c(curr_state)){
    ...
    ### Reset the X_diff y_diff (in this version, we ONLY have histories)
    X_diff <- NULL
    y_diff <- NULL
    ...
    for (fcast_date_idx in 1:(length(fcast_dates_to_match))){
      ...
      if((!added_first_history_flag)&(length(data_till_now$value)>(h+k+1))){
      	...
      	### convert to differences
      	X_diff_temp                          <- t(apply(X_temp,1,diff))
      	y_diff_temp                          <- t(apply(y_temp,1,diff))
      	X_diff <- rbind(X_diff, X_diff_temp)
      	y_diff <- rbind(y_diff, y_diff_temp) 
        ...
        new_y_vals <- data_till_now$value[(length(data_till_now$value)-h):(length(data_till_now$value))]
	      ...
	      ### convert to differences
        X_diff_temp                          <- diff(new_X_vals)
        y_diff_temp                          <- diff(new_y_vals)
        X_diff                               <- rbind(X_diff, X_diff_temp)
        y_diff  			                       <- rbind(y_diff, y_diff_temp)
      } else {
        # In this case, there is not enough data to add to the embedding matrix, and we have yet to add any data.
        # So do nothing, since in this version we ONLY have history.
        next
      }
      ...
      print("dim of embedding is")
      print(dim(X_diff))

      closest_temp = min(length(dist_to_test), closest)

      #### get the closest ids in the X_diff mat
      closest_ids                 <- sort(dist_to_test,index.return = TRUE)$ix[1:closest_temp]
      dists_of_closest            <- sort(dist_to_test)[1:closest_temp]
...
setDefaultCluster(cl)
registerDoParallel(cl)
clusterExport(cl,c('y_diff','X_diff'))
