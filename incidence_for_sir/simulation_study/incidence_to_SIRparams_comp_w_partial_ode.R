...
# TODO: Make sure you're grabbing the flag from optim.  Make sure everything is converging.
...
ggplot(data = df2, aes(x=time))+
        ...
        geom_line(aes(y=incidence), color=I("red")) + # incidence
        geom_point(aes(x = observed_incidence_peak_time, y = observed_peak_incidence))+
  xlim(0,40) + ylim(0,0.07)

# melted_df = melt(df2[,1:5],id=1)
...
#   #geom_point(aes(x = max_incidence_time_df2, y = max_incidence_value_df2), color = 'cyan', size = 3)
