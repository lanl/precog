...
## load libraries
setwd("~/GitLab/incidence_for_sir/application/code")
library(rjags)
library(runjags)
library(ggplot2)
library(reshape2)
...
datapath <- "~/GitLab/incidence_for_sir/application/data/"
jagspath <- "~/GitLab/incidence_for_sir/application/code/"
...
rho_data                = data.frame(variable = rep('rho', times = nrow(incidence_betas)), 
                                     value = 0.9*incidence_betas[c(4)]/incidence_gammas[c(4)])
...
rho_data                 = data.frame(variable = rep('rho', times = nrow(prevalence_betas)), 
                                      value = 0.9*prevalence_betas[c(4)]/prevalence_gammas[c(4)])
