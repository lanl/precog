...
if(!dir.exists(paste0(my_path,"embeddings_lstm"))){dir.create(paste0(my_path,"embeddings_lstm"))}
NUM_RUN = 10
for(i in 1:NUM_RUN){
  cmdLines <- paste0("Rscript --vanilla ",my_path,"/get_lstm_embeddings_internal.R",
                     " --total_run=",NUM_RUN,
                     " --cur_run=",i)
...
cmdLines <- paste0("Rscript --vanilla ",my_path,"/get_lstm_embeddings_internal_m4.R",
                     " --total_run=",1,
                     " --cur_run=",1)
runSlurm <- slurmarray(cmdLines,sname="PP",stime="540",smem="5G",
                         soutdir=paste0('./logs/'), sparallel = 0)
...
### Additional Training Data using M4 Time Series
cmdLines <- paste0("Rscript --vanilla ",my_path,"/get_lstm_embeddings_internal_m4daily.R",
                   " --total_run=",1,
                   " --cur_run=",1)
runSlurm <- slurmarray(cmdLines,sname="PP",stime="540",smem="5G",
                       soutdir=paste0('./logs/'), sparallel = 0)
...
lf <- list.files(paste0(savetrainpath,"embeddings_lstm/"))
...
train_data <- foreach(i=1:length(lf),
                      ...
                      .packages = c('dplyr', 'tsfeatures', 'data.table'))%dopar%{
                        dat = data.table::fread(paste0(paste0(savetrainpath,"embeddings_lstm/"),lf[i]))
...
data.table::fwrite(x=train_final,file =paste0(savetrainpath,"/embeddings_lstm/real_train_mat.csv"),quote = FALSE, row.names = FALSE)
...
lf <- list.files(paste0(savetrainpath,"embeddings_lstm_m4/"))
lf = lf[lf != 'real_train_mat.csv']
lf = lf[!is.na(lf)]
...
train_data <- foreach(i=1:length(lf), 
                      ...
                      #.verbose = T,
                      .packages = c('dplyr', 'tsfeatures', 'data.table'))%dopar%{
                        dat = data.table::fread(paste0(paste0(savetrainpath,"embeddings_lstm_m4/"),lf[i]))
...
train_final = do.call('rbind',train_data) #big big data merge
data.table::fwrite(x=train_final,file =paste0(savetrainpath,"/embeddings_lstm_m4/real_train_mat.csv"),quote = FALSE, row.names = FALSE)
