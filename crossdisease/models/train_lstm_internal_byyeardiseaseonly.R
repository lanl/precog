...
for(year_cutoff in YEAR_RUNS){
  ...
  for(alpha in ALPHA_GRID){
    ...
    if(scale_ts == 'no'){
      if(warm_start == 'yes'){
        FILENAME = paste0(my_path,'/lstm_diseasespecific/fit_',year_cutoff,'_',include_disease,'_', alpha, '_warm_noscale.h5')
      }else{
        FILENAME = paste0(my_path,'/lstm_diseasespecific/fit_',year_cutoff,'_',include_disease,'_', alpha, '_cold_noscale.h5')
      }
    }else{
      if(warm_start == 'yes'){
        FILENAME = paste0(my_path,'/lstm_diseasespecific/fit_',year_cutoff,'_',include_disease,'_', alpha, '_warm.h5')
      }else{
        FILENAME = paste0(my_path,'/lstm_diseasespecific/fit_',year_cutoff,'_',include_disease,'_', alpha, '_cold.h5')
      }
    }
    ...
    if(!file.exists(FILENAME)){
      ...
      if(grepl('Influenza',include_disease, ignore.case = T) & !grepl('Haemophilus',include_disease, ignore.case = T)){
        dat = dat[grepl('Influenza',dat$FILES, ignore.case = T) & !grepl('Influenzae',dat$FILES, ignore.case = T),] 
      }else{
        dat = dat[grepl(include_disease,dat$FILES, ignore.case = T), ] 
      }
      ...
      if(warm_start == 'yes'){
        if(scale_ts == 'no'){
          FILENAME_WARM = paste0(my_path,'/lstm/fit_',year_cutoff,'_', alpha, '_noscale.h5')
        }else{
          FILENAME_WARM = paste0(my_path,'/lstm/fit_',year_cutoff,'_', alpha,'.h5')
        }
