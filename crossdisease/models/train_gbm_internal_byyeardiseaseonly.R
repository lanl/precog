...
for(year_cutoff in YEAR_RUNS){
  ...
  for(alpha in ALPHA_GRID){
    ...
    if(scale_ts == 'no'){
      if(warm_start == 'yes'){
        FILENAME = paste0(my_path,'/results_diseasespecific/fit_',year_cutoff,'_', alpha,'_',include_disease,'_warm_noscale.RDS')
      }else{
        FILENAME = paste0(my_path,'/results_diseasespecific/fit_',year_cutoff,'_', alpha,'_',include_disease,'_cold_noscale.RDS')
      }
    }else{
      if(warm_start == 'yes'){
        FILENAME = paste0(my_path,'/results_diseasespecific/fit_',year_cutoff,'_', alpha,'_',include_disease,'_warm.RDS')
      }else{
        FILENAME = paste0(my_path,'/results_diseasespecific/fit_',year_cutoff,'_', alpha,'_',include_disease,'_cold.RDS')
      }
    }
    if(!file.exists(FILENAME)){
      ...
      if(scale_ts == 'yes'){
        real_data = data.table::fread(file =paste0(savetrainpath,"/../features/results_diseasespecific/",include_disease,"_train_mat.csv"))
      }else{
        real_data = data.table::fread(file =paste0(savetrainpath,"/../features/results_diseasespecific_unscaled/",include_disease,"_train_mat.csv"))
      }
