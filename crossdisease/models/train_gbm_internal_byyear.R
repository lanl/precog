...
for(year_cutoff in YEAR_RUNS){
  ...
  for(alpha in ALPHA_GRID){
    ...
    if(scale_ts == 'no'){
      FILENAME = paste0(my_path,'/results/fit_',year_cutoff,'_', alpha, '_noscale.RDS')
    }else{
      FILENAME = paste0(my_path,'/results/fit_',year_cutoff,'_', alpha, '.RDS')
    }
    
    if(!file.exists(FILENAME) & !file.exists(gsub('.RDS','.txt',FILENAME))){
      if(scale_ts == 'yes'){
        ...
        for(h in 1:4){
          ...
          model_list[[h]] <- lightgbm::lgb.train(params = params,
                                                 ...
                                                 valids = list(train = lgbm_train,
                                                               valid = lgbm_valid),
                                                 early_stopping_rounds = 10, #from 100
                                                 categorical_feature = CAT_VARS,
                                                 nrounds = 5000)
        }
        ## get outta here
...
#results <- mod[[y2]] %>% evaluate(X, y, verbose = 0)
