...
model_path = '~/GitLab/frodo/models/results_diseasespecific/'
...
for(alpha in ALPHA_GRID){
  ...
  for(y in 1:length(YEAR_RUNS)){
    mod[[y]] <- readRDS(paste0(model_path,"fit_",YEAR_RUNS[y],"_",alpha,'_', include_disease,end_key))
  }

  for(i in 1:length(FILES_CUR)){
    
    eval_key = FILES_CUR[i]
    eval_key = gsub('.csv','',eval_key)
    ...
    if(scale_ts == 'no'){
      if(warm_start == 'yes'){
        FILENAME = paste0(my_path,"frodo_noscale/diseasespecific_warm/real_eval_mat_",eval_key_short,"_",alpha, ".csv")
      }else{
        FILENAME = paste0(my_path,"frodo_noscale/diseasespecific_cold/real_eval_mat_",eval_key_short,"_",alpha, ".csv")
      }
    }else{
      if(warm_start == 'yes'){
        FILENAME = paste0(my_path,"frodo/diseasespecific_warm/real_eval_mat_",eval_key_short, "_",alpha,".csv")
      }else{
        FILENAME = paste0(my_path,"frodo/diseasespecific_cold/real_eval_mat_",eval_key_short,"_",alpha,".csv")
      }
    }
