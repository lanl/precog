...
if(scale_ts == 'yes'){
  output_path = '~/GitLab/frodo/evaluations/smoanodiff/all'
}else{
  output_path = '~/GitLab/frodo/evaluations/smoanodiff_noscale/all'
}
...
train_data <- foreach(i=1:length(FILES_CUR), 
  ...
  if(TRUE){
    ...
    for(j in SUBSET_LONG){
      ...
      dists_of_closest = DescTools::Small(dist_to_test, k = closest, unique = FALSE, na.last = NA)
      ...
      closest_ids2 = closest_ids2[1:closest]
