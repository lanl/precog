...
model_path = '~/GitLab/frodo/models/lstm/'
...
for(alpha in ALPHA_GRID){
  ...
  if(scale_ts == 'no'){
    ...
    for(y in 1:length(YEAR_RUNS)){
      ...
      test %>% load_model_weights_hdf5(paste0(model_path,"fit_",YEAR_RUNS[y],'_',alpha,"_noscale.h5"))
      mod[[y]] <- test
    }
  }else{
    mod = replicate(length(YEAR_RUNS), list(NULL))
    for(y in 1:length(YEAR_RUNS)){
      ...
      test %>% load_model_weights_hdf5(paste0(model_path,"fit_",YEAR_RUNS[y],'_',alpha,".h5"))
      mod[[y]] <- test
    }
  }
  ...
  for(i in 1:length(FILES_CUR)){
    ...
    if(scale_ts == 'no'){
      FILENAME = paste0(my_path,"lstm_noscale/all/real_eval_mat_",eval_key,'_', alpha,".csv")
    }else{
      FILENAME = paste0(my_path,"lstm/all/real_eval_mat_",eval_key,'_', alpha,".csv")
    }
        ...
        for(y in 1:length(TO_RUN)){
          ...
          if(nrow(SUBSET)>1){
            ...
            for(j in 1:length(mod[[y2]])){
              pred_list[[j]] <- mod[[y2]] %>% predict(as.matrix(X_real_sub))
              print(j)
            }
