...
model_path = '~/GitLab/frodo/models/lstm_modespecific/'
...
for(alpha in ALPHA_GRID){
  ...
  if(scale_ts == 'no'){
    ...
    for(y in 1:length(YEAR_RUNS)){
      ...
      test %>% load_model_weights_hdf5(paste0(model_path,"fit_",YEAR_RUNS[y],"_",alpha, '_',include_mode,"_noscale.h5"))
      mod[[y]] <- test
    }
  }else{
    mod = replicate(length(YEAR_RUNS), list(NULL))
    for(y in 1:length(YEAR_RUNS)){
      ...
      test %>% load_model_weights_hdf5(paste0(model_path,"fit_",YEAR_RUNS[y],"_",alpha, '_',include_mode,".h5"))
      mod[[y]] <- test
    }
  }
  ...
  for(i in 1:length(FILES_CUR)){
    ...
    if(scale_ts == 'no'){
      if(warm_start == 'yes'){
        FILENAME = paste0(my_path,"lstm_noscale/modespecific_warm/real_eval_mat_",eval_key,'_', alpha,".csv")
      }else{
        FILENAME = paste0(my_path,"lstm_noscale/modespecific_cold/real_eval_mat_",eval_key,'_', alpha,".csv")
      }
    }else{
      if(warm_start == 'yes'){
        FILENAME = paste0(my_path,"lstm/modespecific_warm/real_eval_mat_",eval_key,'_', alpha,".csv")
      }else{
        FILENAME = paste0(my_path,"lstm/modespecific_cold/real_eval_mat_",eval_key,'_', alpha,".csv")
      }
    }
