...
model_path = '~/GitLab/frodo/models/results_modespecific/'
...
FILES_TEMP = gsub('.csv','',unique(gsub('real_train_mat_','',FILES_ALL)))
SPLITS = strsplit(FILES_TEMP, split = '_')
DISEASES = rep(NA, length(FILES_TEMP))
for(i in 1:length(SPLITS)){
  if(length(SPLITS[[i]])==3){
    DISEASES[i] = SPLITS[[i]][2]
  }else if(length(SPLITS[[i]])==4){
    DISEASES[i] = paste0(SPLITS[[i]][2],'_',SPLITS[[i]][3])
  }else if(length(SPLITS[[i]])==5){
    DISEASES[i] = paste0(SPLITS[[i]][2],'_',SPLITS[[i]][3],'_',SPLITS[[i]][4])
  }else{
    DISEASES[i] = paste0(SPLITS[[i]][2],'_',SPLITS[[i]][3],'_',SPLITS[[i]][4],'_',SPLITS[[i]][5])
  }
}
lut_path = '~/GitLab/frodo/lookup_tables/'
...
for(alpha in ALPHA_GRID){
  ...
  for(y in 1:length(YEAR_RUNS)){
    mod[[y]] <- readRDS(paste0(model_path,"fit_",YEAR_RUNS[y],"_",alpha,'_', include_mode,end_key))
  }
  ...
  for(i in 1:length(FILES_CUR)){
    ...
    if(scale_ts == 'no'){
      if(warm_start == 'yes'){
        FILENAME = paste0(my_path,"frodo_noscale/modespecific_warm/real_eval_mat_",eval_key_short,"_",alpha, ".csv")
      }else{
        FILENAME = paste0(my_path,"frodo_noscale/modespecific_cold/real_eval_mat_",eval_key_short,"_",alpha, ".csv")
      }
    }else{
      if(warm_start == 'yes'){
        FILENAME = paste0(my_path,"frodo/modespecific_warm/real_eval_mat_",eval_key_short, "_",alpha,".csv")
      }else{
        FILENAME = paste0(my_path,"frodo/modespecific_cold/real_eval_mat_",eval_key_short,"_",alpha,".csv")
      }
    }
