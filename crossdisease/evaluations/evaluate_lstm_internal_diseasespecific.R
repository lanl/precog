...
option_list <- list(
  ...
  make_option("--include_disease", type="character", default=""),
...
include_disease = as.character(opt$include_disease) 
...
model_path = '~/GitLab/frodo/models/lstm_diseasespecific/'


setwd(my_path)


FILES_LONG = list.files(paste0(feature_path))
...
if(grepl('Influenza',include_disease, ignore.case = T) & !grepl('Haemophilus',include_disease, ignore.case = T)){
  FILES_ALL = FILES_ALL[grepl('Influenza',FILES_ALL, ignore.case = T) & !grepl('Influenzae',FILES_ALL, ignore.case = T)] 
}else{
  FILES_ALL = FILES_ALL[grepl(include_disease,FILES_ALL, ignore.case = T) ] 
}
...
for(alpha in ALPHA_GRID){
  ...
  DISEASES = DISEASES[DISEASES$FILES == include_disease,]
  YEAR_RUNS = c(DISEASES$ML_start:DISEASES$ML_end)
  ...
  if(scale_ts == 'no'){
    ...
    for(y in 1:length(YEAR_RUNS)){
      ...
      test %>% load_model_weights_hdf5(paste0(model_path,"fit_",YEAR_RUNS[y],'_',include_disease,"_",alpha,"_cold_noscale.h5"))
      mod[[y]] <- test
    }
  }else{
    mod = replicate(length(YEAR_RUNS), list(NULL))
    for(y in 1:length(YEAR_RUNS)){
      ...
      test %>% load_model_weights_hdf5(paste0(model_path,"fit_",YEAR_RUNS[y],'_',include_disease,"_",alpha,"_cold.h5"))
      mod[[y]] <- test
    }
  }
  ...
  for(i in 1:length(FILES_CUR)){
    ...
    if(scale_ts == 'no'){
      if(warm_start == 'yes'){
        FILENAME = paste0(my_path,"lstm_noscale/diseasespecific_warm/real_eval_mat_",eval_key,'_', alpha,".csv")
      }else{
        FILENAME = paste0(my_path,"lstm_noscale/diseasespecific_cold/real_eval_mat_",eval_key,'_', alpha,".csv")
      }
    }else{
      if(warm_start == 'yes'){
        FILENAME = paste0(my_path,"lstm/diseasespecific_warm/real_eval_mat_",eval_key,'_', alpha,".csv")
      }else{
        FILENAME = paste0(my_path,"lstm/diseasespecific_cold/real_eval_mat_",eval_key,'_', alpha,".csv")
      }
    }
...
#results <- pmin(pmax(PMIN, mod[[y2]] %>% predict(X)),PMAX)
