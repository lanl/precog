...
library(this.path)
my_path = here::here("epifforma", "uq")
# setwd(my_path)
...
### Equal Weight Predictions in Test Data
results = output_list['plot_df']$plot_df#[,-1]
results = results[results$type == 'fcst',]
results = results[results$model == 'equal_wt',c('ts_length','ts_id','h','fcst','truth')]
...
mod_interval <- fit_lgbm_wt(input =data.frame(interval_long)[TRAIN_INTERVAL==1,c(FEATURE_LIST, 'class', 'class_wt')], nmodels = NMODELS)
saveRDS(mod_interval, file = paste0(savemodelpath,"interval2_equal_wt.RDS"))
