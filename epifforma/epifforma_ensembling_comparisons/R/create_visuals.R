...
point_plot_1 <- ggplot(ggplot_data,
  ...
  geom_label_repel(data = ggplot_data, aes(label = ifelse(model.y%in%models_to_label,as.character(model.y),'')),
                   ...
                   # force_pull = -0.1,
                   size = 5) +
  theme(legend.position = 'none') + xlab("Mean MAE for Model") + ylab("Mean MAE for epiFFORMA") + 
  ...
  annotate("text", x = min(ggplot_data$abs_error_model), y = max(ggplot_data$abs_error_smoa), 
           label = "Model Outperforms epiFFORMA", hjust = 0, vjust = 1, size=6) +  
  annotate("text", x = max(ggplot_data$abs_error_model)-1000, y = min(ggplot_data$abs_error_smoa)+1000, 
           label = "Model Outperformed by epiFFORMA", hjust = 1, vjust = 0, size=6) +
...
point_plot_2 = ggplot(ggplot_data,
  ...
  geom_label_repel(data = ggplot_data, aes(label = ifelse(model.y%in%models_to_label,as.character(model.y),'')),
                   ...
                   force_pull = -0.1,
                   size = 5) +
  theme(legend.position = 'none') + xlab("Mean WIS for Model") + ylab("Mean WIS for epiFFORMA") +
  theme(axis.text=element_text(size=15), axis.title = element_text(size = 18),
        title = element_text(size=15)) +
  theme(panel.spacing = unit(0, "cm"),
        plot.margin = margin(0, 0, 0, 0, "cm"), 
